//  uci.hpp
//  Odin
//  Created by acdemiralp
//  Based on the UCI protocol (April 2006). Extractedfrom: https://github.com/acdemiralp/uci
...
public:
    ...
    static void send_id(const std::string &name = "", const std::string &author = "") {
        std::cout << "id" << (!name.empty() ? " name " + name : "") << (!author.empty() ? " author " + author : "")
                  << std::endl;
    ...
    static void send_option_spin_wheel(const std::string &name, const std::size_t &initial, const std::size_t &minimum,
                                       const std::size_t &maximum) {
        std::cout << "option name " << name << " type spin default " << initial << " min " << minimum << " max "
                  << maximum << std::endl;
    }

    static void send_option_combo_box(const std::string &name, const std::string &initial,
    ...
    static void
    send_option_nalimov_cache(const std::size_t &initial, const std::size_t &minimum, const std::size_t &maximum) {
    ...
    static void send_option_multi_principle_variation(const std::size_t &initial, const std::size_t &minimum,
                                                      const std::size_t &maximum) {
    ...
    static void
    send_option_uci_elo(const std::size_t &initial, const std::size_t &minimum, const std::size_t &maximum) {
                    ...
                    if (token == "searchmoves")
                        while (iss >> token)
                            commands[command::search_moves] +=
                                    std::string(" ", commands[command::search_moves].empty() ? 0 : 1) + token;
