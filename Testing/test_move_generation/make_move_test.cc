...
TEST_CASE("Test MAKE MOVE - EP") {
  ...
  Board actual{makeMove(initial_board, std::make_tuple(36, 43, FIGURES::EMPTY))};
...
TEST_CASE("Test MAKE MOVE - Castle") {
  ...
  Board actual{makeMove(initial_board, std::make_tuple(4, 2, FIGURES::EMPTY))};
  CHECK(actual == comparison);
}
...
TEST_CASE("Test MAKE MOVE - Castle Black") {
  ...
  Board actual{makeMove(initial_board, std::make_tuple(60, 62, FIGURES::EMPTY))};
  std::cout << actual;
  std::cout << comparison;
  CHECK(actual == comparison);
}